              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('Press to send an emergency alert to nearby contacts and authorities', style: TextStyle(fontSize: 12, color: AppColors.textSecondary)),
                  ],
                ),
              ),
  ...
  Future<void> _confirmSos(BuildContext context, WidgetRef ref) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Are you sure you want to send an emergency alert? This will notify your contacts and local authorities.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')),
          TextButton(onPressed: () => Navigator.pop(context, true), child: const Text('Send SOS', style: TextStyle(color: AppColors.error))),
        ],
      ),
    );
    ...
    if (confirmed == true) {
      try {
        await ref.read(safetyProvider.notifier).triggerSos(message: 'Emergency alert from property $propertyId');
        AppToast.success(context, 'Emergency alert sent');
      } catch (e) {
        AppToast.error(context, 'Failed to send emergency alert');
      }
    }
  }
}
